...
```
- And put that inside setup function or set for PageManager
...
-via PageManager:
```dart
   SmartApp.pageManager.go(name: pageName); //Provide the defined name of the target page
```
...
```dart
  ...
  bool mobileMode=SmartApp.Rixa.properties.mobileMode;
